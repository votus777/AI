...
def bulid_model(drop=0.5, optimizer = 'adam') :
    ...
    inputs = Input(shape=(28,28,1 ), name= 'inputs')  # (786,)
    x = Conv2D(12,(3,3), activation='relu', name= 'hidden1')(inputs)
    x = Dropout(drop)(x)
    x = Conv2D(24,(3,3), activation='relu', name = 'hidden2')(x)
    x = Dropout(drop)(x)
    x = MaxPool2D(2,2)(x)
    x = Conv2D(18,(3,3), activation='relu', name = 'hidden3')(x)
    x = Dropout(drop)(x)
    x = Flatten()(x)
    x = Dense(24, activation='relu', name = 'hidden4')(x)
    x = Dropout(drop)(x)
...
'''
{'optimizer': 'rmsprop', 'drop': 0.30000000000000004, 'batch_size': 100}
